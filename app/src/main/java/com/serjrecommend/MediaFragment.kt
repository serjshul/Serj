...
import com.serjrecommend.data.media.MediaAdapter
...
class MediaFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        arguments?.takeIf { it.containsKey(ARG_OBJECT) }?.apply {
            ...
            data = MediaData.getMediaData()
            val adapter = MediaAdapter(data)
            cards.adapter = adapter

            // Set setOnClickListener on cards view
            adapter.setOnClickListener(object : MediaAdapter.OnClickListener {
                override fun onClick(position: Int, model: MediaModel) {
                    // Creating an Intent and sending data to a FullMediaActivity
                    val intent = Intent(view.context, FullMediaActivity::class.java)
